...
class CustomerApprovalController extends Controller
{
    ...
    {
        try {
            $settings = CustomerApprovalSetting::first();

            $approvers = CustomerApprovalApprover::with('user:id,name,email,avatar')
                ->orderBy('sort_order')
                ->get()
                ->map(fn ($a) => $a->user)
                ->filter()
            ...
            $hierarchy = CustomerApprovalHierarchyLevel::with('user:id,name,email,avatar')
                ->orderBy('level_order')
                ->get()
                ->map(fn ($h) => [
                    'id'          => $h->id,
            ...
            $this->logException('CustomerApprovalController::show', $e, $this->buildCtx($request, 'CustomerApprovalController::show'));
            return $this->errorResponse('Failed to fetch approval settings.', 500);
        }
    }

    // ── PUT /api/approvals/customers ─────────────────────────────────────────

    public function update(Request $request): JsonResponse
    {
        $ctx = $this->buildCtx($request, 'CustomerApprovalController::update');
        ...
        try {
            DB::transaction(function () use ($validated, $request) {
                $old = CustomerApprovalSetting::first();

                $settings = CustomerApprovalSetting::updateOrCreate([], [
                ...
                try {
                    AuditLog::create([
                        'auditable_type' => 'CustomerApprovalSetting',
            ...
            $this->logException('CustomerApprovalController::update', $e, $ctx);
