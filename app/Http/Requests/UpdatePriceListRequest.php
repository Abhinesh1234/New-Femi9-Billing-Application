...
class UpdatePriceListRequest extends FormRequest
{
    ...
    {
        return [
            // ── Core (all optional on update) ─────────────────────────────────
            'name'                   => 'sometimes|required|string|min:1|max:255',
            'transaction_type'       => 'sometimes|required|in:sales,purchase,both',
            'customer_category_id'   => 'nullable|integer',
            'price_list_type'        => 'sometimes|required|in:all_items,individual_items',
        ...
        $validator->after(function (Validator $v) {
            ...
            foreach ($items as $i => $item) {
                ...
                if (in_array($id, $seenItemIds, true)) {
                    $v->errors()->add("items.{$i}.item_id", 'This item is listed more than once. Adjust quantity or remove the duplicate.');
                } else {
                    $seenItemIds[] = $id;
                }
            }

            // Rule 2: per-item volume ranges — end_qty must be >= start_qty
            foreach ($items as $i => $item) {
