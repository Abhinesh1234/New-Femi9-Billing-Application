...
import { fetchItemTransactions, type TransactionRow, type TransactionStatus, type TransactionMeta } from "../../../../core/services/itemTransactionsApi";
...
const ItemOverview = () => {
  ...
  // ── Transactions tab ──────────────────────────────────────────────────────
  const [txnStatus,   setTxnStatus]   = useState<TransactionStatus>("all");
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setTxnLoading(true);
    fetchItemTransactions(itemId, { status: txnStatus, page: txnPage, per_page: 50 }).then(res => {
      if (cancelled) return;
      if (res.success) { setTxnRows(res.data); setTxnMeta(res.meta); }
      setTxnLoading(false);
    });
    return () => { cancelled = true; };
  }, [activeTab, id, txnStatus, txnPage]);
                      ...
                      <div className="d-flex align-items-center justify-content-between flex-wrap gap-2">
                        ...
                        <div className="d-flex align-items-center gap-2">
                          ...
                          <select
                            ...
                            value={txnStatus}
                            onChange={e => { setTxnStatus(e.target.value as TransactionStatus); setTxnPage(1); }}
                          >
                            ...
                            <tbody>
                              {txnRows.map((row, i) => {
                                const statusMap: Record<string, { label: string; color: string; bg: string }> = {
                                  ...
                                  sent:           { label: "Confirmed",     color: "#0d6efd", bg: "#e7f1ff" },
                                  paid:           { label: "Paid",          color: "#198754", bg: "#e8f5e9" },
                                  partially_paid: { label: "Partial",       color: "#fd7e14", bg: "#fff3e0" },
                                  overdue:        { label: "Overdue",       color: "#dc3545", bg: "#fdecea" },
                                  void:           { label: "Void",          color: "#6c757d", bg: "#f8f9fa" },
                                };
                                const st = statusMap[row.status] ?? { label: row.status, color: "#444", bg: "#f0f1f3" };
                                return (
                                  <tr key={`${row.invoice_id}-${i}`} style={{ borderBottom: "1px solid #f5f5f5" }}>
                                    ...
                                    <td style={{ padding: "12px 16px", verticalAlign: "middle" }}>
                                      <span className="fs-12 fw-semibold px-2 py-1 rounded" style={{ color: st.color, background: st.bg }}>
                                        {st.label}
                                      </span>
                                    </td>
                                  </tr>
