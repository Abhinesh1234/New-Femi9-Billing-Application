...
const ManageItemsModal = ({
  ...
  onSave, onUpdate, onDelete, onSaveAndSelect, icon = "ti-list-details",
  ...
  return (
    <Modal show={show} onHide={onHide} centered size="lg">
      <Modal.Header style={{ padding: "20px 24px 18px", borderBottom: "1px solid #f1f5f9", gap: 12 }}>
        <div style={{ width: 42, height: 42, borderRadius: "50%", background: "#fef2f2", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          <i className={`ti ${icon} fs-18`} style={{ color: "#ef4444" }} />
        </div>
        <span style={{ fontWeight: 600, fontSize: 16, color: "#0f172a", flex: 1 }}>{title}</span>
        <button
          type="button"
          onClick={onHide}
  ...
  return (
    <Modal show={show} onHide={onHide} centered size="lg">
      <Modal.Header style={{ padding: "20px 24px 18px", borderBottom: "1px solid #f1f5f9", gap: 12 }}>
        ...
        <span style={{ fontWeight: 600, fontSize: 16, color: "#0f172a", flex: 1 }}>
...
const ManageCategoriesModal = ({
  ...
  return (
    <Modal show={show} onHide={onHide} centered size="lg">
      <Modal.Header style={{ padding: "20px 24px 18px", borderBottom: "1px solid #f1f5f9", gap: 12 }}>
        <div style={{ width: 42, height: 42, borderRadius: "50%", background: "#fef2f2", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          <i className="ti ti-category fs-18" style={{ color: "#ef4444" }} />
        </div>
        <span style={{ fontWeight: 600, fontSize: 16, color: "#0f172a", flex: 1 }}>Manage Categories</span>
        <button
          type="button"
          onClick={onHide}
          style={{ width: 32, height: 32, borderRadius: "50%", border: "1.5px solid #fecaca", background: "#fef2f2", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0, padding: 0 }}
...
const NewItem = () => {
  ...
  const handleRefresh = useCallback(async () => {
    ...
    setEditLoading(true);
    try {
      // Bust caches then re-fetch all reference option lists
      ...
      // Hard-clear selections whose referenced record was deleted; update label if renamed
      ...
      setErrors({});
      showToast("success", "Reference data refreshed.");
    } catch {
      showToast("danger", "Failed to refresh. Please try again.");
    } finally {
      setEditLoading(false);
    }
  }, [isEditMode, editId, brandId, brand, categoryId, category, hsnCodeId, hsnCode, gstRateId, gstValue, salesAccountId, salesAccount, purchaseAccountId, purchaseAccount, inventoryAccountId, inventoryAccount]);
