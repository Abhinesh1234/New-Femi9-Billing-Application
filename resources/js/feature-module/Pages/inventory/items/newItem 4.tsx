...
const EntityField = ({ label, value, onChange, items, onManage }: EntityFieldProps) => {
  ...
  const computeStyle = (): React.CSSProperties => {
    ...
    return dropUp
      ? { position: "fixed", bottom: window.innerHeight - rect.top + 4, left: rect.left, width: rect.width, zIndex: 9999, minWidth: 220 }
      : { position: "fixed", top: rect.bottom + 4, left: rect.left, width: rect.width, zIndex: 9999, minWidth: 220 };
  };

  const toggle = () => {
    if (!open) setMenuStyle(computeStyle());
    setOpen((o) => !o);
    setSearch("");
  };
  ...
  useEffect(() => {
    if (!open) return;
    const update = () => setMenuStyle(computeStyle());
...
const NewItem = () => {
  ...
  const handleRefresh = useCallback(async () => {
    ...
    dispatch(startLoading("edit-item"));
    try {
      ...
      dispatch(stopLoading("edit-item"));
    }
