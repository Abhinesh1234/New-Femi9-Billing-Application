...
import { storeItem, uploadItemImage, uploadCustomFieldFile, fetchItems, type ItemPayload } from "../../../../core/services/itemApi";
        ...
        {showNewForm && (
          <div className="px-4 pt-3 pb-3 border-bottom">
            ...
            <div className="d-flex gap-2">
              <button type="button" className="btn btn-primary" onClick={handleSaveAndSelect} disabled={modalSaving}>
            ...
            <div key={item.id}>
              {editingId === item.id ? (
                <div className="py-3 border-bottom">
                  ...
                  />
                  <div className="d-flex gap-2">
                    <button type="button" className="btn btn-primary" onClick={commitEdit} disabled={modalSaving}>
  ...
  return (
    <Modal show={show} onHide={onHide} centered size="lg">
      ...
      <Modal.Footer className="px-4 py-3 justify-content-start">
        <button type="button" className="btn btn-primary me-2" onClick={handleApply}>
          Generate SKU
        </button>
...
const NewItem = () => {
  ...
  useEffect(() => {
    fetchCustomFields("products").then((res) => {
      if (res.success) {
        const fields = res.data.filter((f) => f.config.is_active);
    ...
    fetchSettings<ProductConfiguration>("products").then((res) => {
      if (res.success && res.configuration) {
        if (res.configuration.dimension_unit) setDimUnit(res.configuration.dimension_unit as DimensionUnit);
        if (res.configuration.weight_unit)    setWeightUnit(res.configuration.weight_unit as WeightUnit);
      }
    });
    fetchBrands().then((res) => {
      if (res.success) setBrands((res.data as Brand[]).map((b) => ({ id: b.id, name: b.name })));
    });
    fetchCategories().then((res) => {
      if (res.success) setCategories((res.data as Category[]).map((c) => ({ id: c.id, name: c.name, parentId: c.parent_id })));
    });
    fetchHsnCodes().then((res) => {
      if (res.success) setHsnCodes((res.data as HsnCode[]).map((h) => ({ id: h.id, name: h.code })));
    });
    fetchGstRates().then((res) => {
      if (res.success) setGstValues((res.data as GstRate[]).map((g) => ({ id: g.id, name: g.label })));
    });
    fetchAccounts("sales").then((res) => {
      if (res.success) setSalesAccounts((res.data as Account[]).map((a) => ({ id: a.id, name: a.name })));
    });
    fetchAccounts("purchase").then((res) => {
      if (res.success) setPurchaseAccounts((res.data as Account[]).map((a) => ({ id: a.id, name: a.name })));
    });
    fetchAccounts("inventory").then((res) => {
      if (res.success) setInventoryAccounts((res.data as Account[]).map((a) => ({ id: a.id, name: a.name })));
    });
  }, []);
  ...
  const renderCfInput = (field: CustomField, value: string, onChange: (v: string) => void) => {
    ...
    switch (data_type) {
      ...
      case "auto_generate": {
        const prefix  = (tc.prefix  as string | null) || "";
        const suffix  = (tc.suffix  as string | null) || "";
        const start   = (tc.starting_number as number) ?? 1;
        const padded  = String(start).padStart(3, "0");
  ...
  const handleSave = async () => {
    ...
    const res = await storeItem(payload);
    setSaving(false);

    if (res.success) {
      showToast("success", res.message || "Item saved successfully.");
      setTimeout(() => navigate("/items", { replace: true }), 1200);
    } else {
      ...
      <div className="page-wrapper">
        <div className="content">
          ...
          <div className="d-flex align-items-center justify-content-between mb-4">
            <h4 className="fw-semibold fs-18 mb-0">New Item</h4>
            <button
              type="button"
            ...
            <div className="card-body p-4">
                  ...
                  <>
                    ...
                    {formItemType === "single" ? (
                      <div className="row mb-3 align-items-center">
                        ...
                        <label className="col-sm-2 col-form-label text-danger fw-medium fs-14 text-end">
                          Account <span>*</span>
                        </label>
              ...
              {itemType === "goods" && (
                <div className="border-top pt-4 mb-4">
                  ...
                  <div className="row mb-3 align-items-center">
                    ...
                    <label className="col-sm-2 col-form-label fw-medium fs-14 text-end">Weight</label>
                    <div className="col-sm-4">
                      <div className="input-group">
                        <input type="number" className="form-control"
              ...
              <div className="border-top pt-4 mb-4">
                ...
                <div className="row mb-3 align-items-center">
                  ...
                  <label className="col-sm-2 col-form-label fw-medium fs-14 text-end">GST</label>
                  <div className="col-sm-4">
                    <EntityField
                      label="GST"
                      value={gstValue}
                    ...
                    if (isFullWidth) {
                      ...
                      rows.push(
                        <div key={field.id} className="row mb-3 align-items-center">
                          ...
                          {hasPair && next && (
                            <>
                              <label className={`col-sm-2 col-form-label fw-medium fs-14 text-end${next.config.is_mandatory || cfErrors[next.config.field_key] ? " text-danger" : ""}`}>
                                {next.config.label}
              ...
              <div className="border-top pt-3 d-flex align-items-center gap-2">
                ...
                <button
                  type="button"
                  className="btn btn-cancel btn-sm px-4"
                  onClick={() => window.history.length > 1 ? navigate(-1) : navigate("/")}
                  disabled={saving}
                >
                  Cancel
