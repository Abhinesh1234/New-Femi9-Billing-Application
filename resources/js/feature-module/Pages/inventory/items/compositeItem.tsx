...
type CompositeType = "assembly" | "kit";
interface AssociatedRow { id: number; item_id: number | null; item_name: string; item_image: string | null; quantity: string; selling_price: string; cost_price: string; }
...
const CompositeItem = () => {
  ...
  const [associatedRows, setAssociatedRows]     = useState<AssociatedRow[]>([
    { id: 1, item_id: null, item_name: "", item_image: null, quantity: "1", selling_price: "0.00", cost_price: "0.00" },
  ]);
  const assocNextId = useRef(2);
  const addAssocRow = () => setAssociatedRows((p) => [
    ...p, { id: assocNextId.current++, item_id: null, item_name: "", item_image: null, quantity: "1", selling_price: "0.00", cost_price: "0.00" },
  ]);
  const removeAssocRow = (id: number) => setAssociatedRows((p) => p.filter((r) => r.id !== id));
  const updateAssocRow = (id: number, field: keyof Omit<AssociatedRow, "id">, value: string | number | null) =>
    setAssociatedRows((p) => p.map((r) => r.id === id ? { ...r, [field]: value } : r));
  ...
  const assocTotalSelling = associatedRows.reduce((s, r) => s + (parseFloat(r.selling_price) || 0) * (parseFloat(r.quantity) || 0), 0);
  const assocTotalCost    = associatedRows.reduce((s, r) => s + (parseFloat(r.cost_price)    || 0) * (parseFloat(r.quantity) || 0), 0);

  // ── Associate Items dropdown search ───────────────────────────────────────
  const [assocOpenId, setAssocOpenId]         = useState<number | null>(null);
  const [assocSearchText, setAssocSearchText] = useState<Record<number, string>>({});
  const [assocDropItems, setAssocDropItems]   = useState<ItemListRecord[]>([]);
  const [assocDropLoading, setAssocDropLoading] = useState(false);
  const [assocDropPos, setAssocDropPos]       = useState<{ top: number; left: number; width: number } | null>(null);
  const [assocImgErrors, setAssocImgErrors]   = useState<Record<number, boolean>>({});
  const assocSearchTimer  = useRef<ReturnType<typeof setTimeout> | null>(null);
  const assocWrapRefs     = useRef<Record<number, HTMLDivElement | null>>({});
  const assocPortalRef    = useRef<HTMLDivElement>(null);

  const loadAssocDropItems = useCallback(async (q: string) => {
    setAssocDropLoading(true);
    const res = await fetchItems({
      search: q,
      per_page: 30,
      ...(compositeType === "assembly" ? { exclude_composite: true } : {}),
    });
    setAssocDropItems(res.success ? res.data.data : []);
    setAssocDropLoading(false);
  }, [compositeType]);

  const loadSvcDropItems = useCallback(async (q: string) => {
    setSvcDropLoading(true);
    const res = await fetchItems({ search: q, per_page: 30, item_type: "service" });
    setSvcDropItems(res.success ? res.data.data : []);
    setSvcDropLoading(false);
  }, []);

  const toggleAssocDrop = (rowId: number) => {
    if (assocOpenId === rowId) { setAssocOpenId(null); setAssocDropPos(null); return; }
    const el = assocWrapRefs.current[rowId];
    if (el) {
    ...
    setAssocSearchText((s) => ({ ...s, [rowId]: "" }));
    setAssocOpenId(rowId);
    loadAssocDropItems("");
  };

  const clearAssocRow = (rowId: number) => {
    updateAssocRow(rowId, "item_id", null);
    updateAssocRow(rowId, "item_name", "");
    updateAssocRow(rowId, "item_image", null);
    updateAssocRow(rowId, "selling_price", "0.00");
    updateAssocRow(rowId, "cost_price", "0.00");
    setAssocSearchText((s) => { const n = { ...s }; delete n[rowId]; return n; });
    setAssocImgErrors((s) => { const n = { ...s }; delete n[rowId]; return n; });
  };

  // Close dropdown on outside click
  useEffect(() => {
    if (assocOpenId === null) return;
    const handler = (e: MouseEvent) => {
      const wrapEl   = assocWrapRefs.current[assocOpenId];
      const portalEl = assocPortalRef.current;
      if (
        wrapEl && !wrapEl.contains(e.target as Node) &&
        (!portalEl || !portalEl.contains(e.target as Node))
      ) {
        setAssocOpenId(null);
        setAssocDropPos(null);
      }
    };
    document.addEventListener("mousedown", handler);
    return () => document.removeEventListener("mousedown", handler);
  ...
  // ── Associate Services state ──────────────────────────────────────────────
  const [showAssocServices, setShowAssocServices] = useState(false);
  const [associatedServiceRows, setAssociatedServiceRows] = useState<AssociatedRow[]>([
    { id: 1, item_id: null, item_name: "", item_image: null, quantity: "1", selling_price: "0.00", cost_price: "0.00" },
  ]);
  const svcNextId = useRef(2);
  const addServiceRow = () => setAssociatedServiceRows((p) => [
    ...p, { id: svcNextId.current++, item_id: null, item_name: "", item_image: null, quantity: "1", selling_price: "0.00", cost_price: "0.00" },
  ]);
  const removeServiceRow = (id: number) => setAssociatedServiceRows((p) => p.filter((r) => r.id !== id));
  const updateServiceRow = (id: number, field: keyof Omit<AssociatedRow, "id">, value: string | number | null) =>
    setAssociatedServiceRows((p) => p.map((r) => r.id === id ? { ...r, [field]: value } : r));
  const svcTotalSelling = associatedServiceRows.reduce((s, r) => s + (parseFloat(r.selling_price) || 0) * (parseFloat(r.quantity) || 0), 0);
  const svcTotalCost    = associatedServiceRows.reduce((s, r) => s + (parseFloat(r.cost_price)    || 0) * (parseFloat(r.quantity) || 0), 0);

  const [svcOpenId, setSvcOpenId]       = useState<number | null>(null);
  const [svcSearchText, setSvcSearchText] = useState<Record<number, string>>({});
  const [svcDropPos, setSvcDropPos]     = useState<{ top: number; left: number; width: number } | null>(null);
  const [svcDropItems, setSvcDropItems]   = useState<ItemListRecord[]>([]);
  const [svcDropLoading, setSvcDropLoading] = useState(false);
  const svcWrapRefs   = useRef<Record<number, HTMLDivElement | null>>({});
  const svcPortalRef  = useRef<HTMLDivElement>(null);
  const svcSearchTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const [svcImgErrors, setSvcImgErrors] = useState<Record<number, boolean>>({});

  const toggleSvcDrop = (rowId: number) => {
    if (svcOpenId === rowId) { setSvcOpenId(null); setSvcDropPos(null); return; }
    const el = svcWrapRefs.current[rowId];
    if (el) {
      const rect = el.getBoundingClientRect();
      setSvcDropPos({
        top:   rect.bottom + window.scrollY + 4,
    ...
    setSvcSearchText((s) => ({ ...s, [rowId]: "" }));
    setSvcOpenId(rowId);
    loadSvcDropItems("");
  };

  const clearSvcRow = (rowId: number) => {
    updateServiceRow(rowId, "item_id", null);
    updateServiceRow(rowId, "item_name", "");
    updateServiceRow(rowId, "item_image", null);
    updateServiceRow(rowId, "selling_price", "0.00");
    updateServiceRow(rowId, "cost_price", "0.00");
    setSvcSearchText((s) => { const n = { ...s }; delete n[rowId]; return n; });
    setSvcImgErrors((s) => { const n = { ...s }; delete n[rowId]; return n; });
  };

  useEffect(() => {
    if (svcOpenId === null) return;
    const handler = (e: MouseEvent) => {
      const wrapEl   = svcWrapRefs.current[svcOpenId];
      const portalEl = svcPortalRef.current;
      if (
        wrapEl && !wrapEl.contains(e.target as Node) &&
        (!portalEl || !portalEl.contains(e.target as Node))
      ) {
    ...
    const load = async () => {
      ...
      if (isEditMode && editId) {
        ...
        if (itemRes.success) {
          ...
          setName(d.name ?? "");
          setCompositeType((d.composite_type ?? "assembly") as CompositeType);
          ...
          // ── Components (associated items / services) ───────────────────────
          const components = (d.components ?? []) as CompositeComponentRecord[];
          const itemComps  = components.filter((c) => c.component_type === "item");
          const svcComps   = components.filter((c) => c.component_type === "service");

          if (itemComps.length > 0) {
            setAssociatedRows(itemComps.map((c, i) => ({
              id:            i + 1,
              item_id:       c.component_item_id,
              item_name:     c.component_item?.name ?? "",
              item_image:    toStorageUrl(c.component_item?.image ?? null),
              quantity:      c.quantity,
              selling_price: c.selling_price ?? "0.00",
              cost_price:    c.cost_price    ?? "0.00",
            })));
            assocNextId.current = itemComps.length + 1;
          }

          if (svcComps.length > 0) {
            setShowAssocServices(true);
            setAssociatedServiceRows(svcComps.map((c, i) => ({
              id:            i + 1,
              item_id:       c.component_item_id,
              item_name:     c.component_item?.name ?? "",
              item_image:    toStorageUrl(c.component_item?.image ?? null),
            ...
            svcNextId.current = svcComps.length + 1;
          }
  ...
  const validate = (): boolean => {
    ...
    // ── Core ──────────────────────────────────────────────────────────────────
    if (!name.trim())  errs.name = "Composite item name is required.";
    if (!compositeType) errs.compositeType = "Please select a composite type (Assembly or Kit).";

    // ── Associated items: at least one must have a real item selected ──────────
    const filledItems = associatedRows.filter((r) => r.item_id !== null);
    if (filledItems.length === 0) {
      errs.components = "At least one associated item must be selected.";
    } else {
      // Per-row: quantity > 0
      associatedRows.forEach((r) => {
        if (r.item_id !== null && (parseFloat(r.quantity) || 0) <= 0) {
          errs[`assocQty_${r.id}`] = "Quantity must be greater than 0.";
        }
      });
    }

    // ── Service rows (only when shown) ────────────────────────────────────────
    if (showAssocServices) {
      associatedServiceRows.forEach((r) => {
        if (r.item_id !== null && (parseFloat(r.quantity) || 0) <= 0) {
          errs[`svcQty_${r.id}`] = "Quantity must be greater than 0.";
        }
      });
    }
    ...
    if (compositeType !== "kit" && hasPurchaseInfo && !purchaseAccountId) errs.purchaseAccount = "Purchase account is required.";
  ...
  const handleSave = async () => {
    ...
    const itemComponents = associatedRows
      .filter((r) => r.item_id !== null)
      .map((r, i) => ({
        component_item_id: r.item_id!,
        component_type:    "item" as const,
        quantity:          Math.max(parseFloat(r.quantity) || 1, 0.0001),
        selling_price:     parseFloat(r.selling_price) > 0 ? parseFloat(r.selling_price) : null,
        cost_price:        parseFloat(r.cost_price)    > 0 ? parseFloat(r.cost_price)    : null,
        sort_order:        i,
      }));

    const svcComponents = (showAssocServices ? associatedServiceRows : [])
      .filter((r) => r.item_id !== null)
      .map((r, i) => ({
        component_item_id: r.item_id!,
        ...
        selling_price:     parseFloat(r.selling_price) > 0 ? parseFloat(r.selling_price) : null,
        cost_price:        parseFloat(r.cost_price)    > 0 ? parseFloat(r.cost_price)    : null,
        sort_order:        itemComponents.length + i,
      }));

    const payload: CompositeItemPayload = {
      name:           name.trim(),
      ...
      refs: {
        ...
        purchase_account_id:  compositeType !== "kit" ? purchaseAccountId : null,
        inventory_account_id: compositeType !== "kit" ? inventoryAccountId : null,
      },
      has_sales_info:       hasSalesInfo,
      selling_price:        hasSalesInfo && sellingPrice ? parseFloat(sellingPrice) : null,
      sales_description:    hasSalesInfo && salesDesc.trim() ? salesDesc.trim() : null,
      has_purchase_info:    compositeType !== "kit" && hasPurchaseInfo,
      cost_price:           compositeType !== "kit" && hasPurchaseInfo && costPrice ? parseFloat(costPrice) : null,
      purchase_description: compositeType !== "kit" && hasPurchaseInfo && purchaseDesc.trim() ? purchaseDesc.trim() : null,
      preferred_vendor:     compositeType !== "kit" ? (preferredVendor || null) : null,
      track_inventory:      itemType === "goods" && compositeType !== "kit" ? trackInventory : false,
      valuation_method:     itemType === "goods" && compositeType !== "kit" && trackInventory && valuationMethod
      ...
      components:           [...itemComponents, ...svcComponents],
    };
    ...
    if (res.success) {
      ...
      showToast("danger", res.message || "Failed to save composite item.");
      if ("errors" in res && res.errors) {
        ...
        Object.entries(res.errors).forEach(([k, msgs]) => {
          ...
          if (compMatch) {
            ...
            const row   = associatedRows[idx];
            if (row) apiErrs[`assocRow_${row.id}_${field}`] = msgs[0];
          } else {
    ...
    <>
      <div className="page-wrapper">
        <div className="content">
          ...
          <PageHeader
            title={isEditMode ? "Edit Composite Item" : "New Composite Item"}
            ...
            <div className="card-body p-4">
                ...
                <div className="col-lg-6">
                  ...
                  <div className="row mb-3 align-items-start">
                    ...
                    <div className="col-sm-8">
                      {([
                        {
                          value: "assembly" as CompositeType,
                          label: "Assembly Item",
                          desc:  "A group of items combined together to be tracked and managed as a single item.",
                        },
                        {
                          value: "kit" as CompositeType,
                          label: "Kit Item",
                          desc:  "Individual items sold together as one kit.",
                        },
                      ]).map((opt) => {
                        const active = compositeType === opt.value;
                        return (
                          <div
                            key={opt.value}
                            className="d-flex align-items-start gap-2 mb-2"
                            style={{ cursor: "pointer" }}
                            onClick={() => { setCompositeType(opt.value); clr("compositeType"); }}
                          >
                            <input
                              ...
                              name="compositeType"
                              checked={active}
                              onChange={() => { setCompositeType(opt.value); clr("compositeType"); }}
                              onClick={(e) => e.stopPropagation()}
                            />
                            <div>
                              <div className="fw-medium fs-14" style={{ color: active ? "var(--bs-danger, #dc3545)" : "#212529" }}>
                                {opt.label}
                              </div>
                              <div className="fs-13 text-muted">{opt.desc}</div>
                            </div>
                          </div>
                        );
                      })}
                      {errors.compositeType && (
                        <div className="text-danger fs-12 mt-1">
                          <i className="ti ti-alert-circle me-1" />{errors.compositeType}
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div className="border-top pt-4 mb-4">
                <h6 className="fw-semibold fs-15 mb-3">
                  Associate Items{"\u00A0"}<span className="text-danger">*</span>
                </h6>

                <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" as any }}>
                <div className="border rounded overflow-hidden" style={{ minWidth: 560 }}>
                  ...
                  {/* Rows */}
                  {/* Section-level error: no item selected at all */}
                  {errors.components && (
                    <div className="px-3 pt-2 pb-1">
                      <span className="text-danger fs-12"><i className="ti ti-alert-circle me-1" />{errors.components}</span>
                    </div>
                  )}
                  {associatedRows.map((row) => (
                    <div key={row.id} className="d-flex align-items-center px-3 py-3 border-bottom" style={{ gap: 12 }}>
                      {/* Image — hidden on xs, shown sm+ */}
                      <div className="d-none d-sm-block" style={{ width: 68, flexShrink: 0 }}>
                        <div
                        ...
                        >
                          {row.item_image && !assocImgErrors[row.id] ? (
                            <img
                              src={row.item_image}
                              alt=""
                              style={{ width: "100%", height: "100%", objectFit: "cover" }}
                              onError={() => setAssocImgErrors((p) => ({ ...p, [row.id]: true }))}
                            />
                          ) : (
                      ...
                      {/* Item name — EntityField-style selector */}
                      <div
                        style={{ flex: 3 }}
                        ref={(el) => { assocWrapRefs.current[row.id] = el; }}
                      >
                        <div className={`input-group${errors[`assocRow_${row.id}_component_item_id`] ? " is-invalid" : ""}`}>
                          <input
                            type="text"
                            className={`form-control${errors[`assocRow_${row.id}_component_item_id`] ? " is-invalid" : ""}`}
                            placeholder="Select or Add Item"
                            value={row.item_name}
                            readOnly
                            style={{ cursor: "pointer" }}
                            onClick={() => { toggleAssocDrop(row.id); clr(`assocRow_${row.id}_component_item_id`); clr("components"); }}
                          />
                          {row.item_name && (
                            <button
                              type="button"
                          ...
                          <button
                            ...
                            onClick={() => { toggleAssocDrop(row.id); clr(`assocRow_${row.id}_component_item_id`); clr("components"); }}
                          >
                            <i className={`ti ti-chevron-${assocOpenId === row.id ? "up" : "down"}`} />
                          </button>
                        </div>
                        {errors[`assocRow_${row.id}_component_item_id`] && (
                          <div className="text-danger fs-12 mt-1">
                      ...
                      <div style={{ width: 80, flexShrink: 0 }}>
                        <input
                          type="number"
                          className={`form-control text-center${errors[`assocQty_${row.id}`] ? " is-invalid" : ""}`}
                          min={0.0001}
                          step="any"
                          value={row.quantity}
                          onChange={(e) => { updateAssocRow(row.id, "quantity", e.target.value); clr(`assocQty_${row.id}`); }}
                          onBlur={(e) => {
                            const v = parseFloat(e.target.value);
                            if (!e.target.value.trim() || isNaN(v) || v <= 0)
                              updateAssocRow(row.id, "quantity", "1");
                          }}
                        />
                        {errors[`assocQty_${row.id}`] && (
                          <div className="text-danger fs-12 mt-1">{errors[`assocQty_${row.id}`]}</div>
                        )}
                      </div>
                      ...
                      {/* Actions */}
                      <div className="d-flex gap-2 align-items-center" style={{ width: 40, flexShrink: 0 }}>
                        <button
                          type="button"
                          className="btn p-0 border-0 bg-transparent text-danger"
                          title="Remove"
                          disabled={associatedRows.length === 1}
                  ...
                  <div className="px-3 pt-2 pb-1" style={{ background: "#fafafa" }}>
                    {/* Buttons row */}
                    <div className="d-flex align-items-center gap-3 mb-2">
                      ...
                      {!showAssocServices && (
                        <button
                          ...
                          className="btn btn-link p-0 fs-13 text-danger d-flex align-items-center gap-1"
                          style={{ textDecoration: "none", whiteSpace: "nowrap" }}
                          onClick={() => setShowAssocServices(true)}
                        >
                          <i className="ti ti-circle-plus fs-15" />
                          Add Services
                        </button>
                    ...
                    {/* Totals row — column-aligned */}
                    <div className="d-flex align-items-center pb-2" style={{ gap: 12 }}>
                      <div className="d-none d-sm-block" style={{ width: 68, flexShrink: 0 }} />
                      ...
                      <div style={{ flex: 2, minWidth: 0 }}>
                        <span className="fs-14 fw-semibold">{assocTotalSelling.toFixed(2)}</span>
                      </div>
                      <div style={{ flex: 2, minWidth: 0 }}>
                        <span className="fs-14 fw-semibold">{assocTotalCost.toFixed(2)}</span>
                      </div>
                      <div style={{ width: 40, flexShrink: 0 }} />
                    </div>
                  </div>
                ...
                <div className="border-top pt-4 mb-4">
                  <div className="d-flex align-items-center justify-content-between mb-3">
                    <h6 className="fw-semibold fs-15 mb-0">
                      Associate Services{"\u00A0"}<span className="text-danger">*</span>
                    </h6>
                    <button
                      type="button"
                      ...
                      onClick={() => {
                        setShowAssocServices(false);
                        setAssociatedServiceRows([{ id: 1, item_id: null, item_name: "", item_image: null, quantity: "1", selling_price: "0.00", cost_price: "0.00" }]);
                        svcNextId.current = 2;
                        setSvcSearchText({});
                        setSvcImgErrors({});
                      }}
                  ...
                  <div className="border rounded overflow-hidden" style={{ minWidth: 560 }}>
                    ...
                    <div className="d-flex align-items-start px-3 py-2 border-bottom" style={{ background: "#f8f9fa", gap: 12 }}>
                      ...
                      <div style={{ flex: 3 }}>
                        <span className="fw-semibold fs-12 text-uppercase">Service Details</span>
                      </div>
                      <div style={{ width: 80, flexShrink: 0 }}>
                        <span className="fw-semibold fs-12 text-uppercase">Qty</span>
                      </div>
                      ...
                      <div key={row.id} className="d-flex align-items-center px-3 py-3 border-bottom" style={{ gap: 12 }}>
                        ...
                        <div className="d-none d-sm-block" style={{ width: 68, flexShrink: 0 }}>
                          <div
                            ...
                            style={{ width: 64, height: 64, background: "#f8f9fa" }}
                          >
                            {row.item_image && !svcImgErrors[row.id] ? (
                              <img
                                src={row.item_image}
                                alt=""
                                style={{ width: "100%", height: "100%", objectFit: "cover" }}
                                onError={() => setSvcImgErrors((p) => ({ ...p, [row.id]: true }))}
                              />
                            ) : (
                              <i className="ti ti-photo text-muted" style={{ fontSize: 22 }} />
                            )}
                        ...
                        <div
                          style={{ flex: 3 }}
                          ref={(el) => { svcWrapRefs.current[row.id] = el; }}
                        >
                          <div className="input-group">
                            <input
                              type="text"
                              ...
                              readOnly
                              style={{ cursor: "pointer" }}
                              onClick={() => toggleSvcDrop(row.id)}
                            />
                            {row.item_name && (
                              <button type="button" className="btn btn-outline-light" title="Clear" onClick={() => clearSvcRow(row.id)}>
                                <i className="ti ti-x" style={{ fontSize: 13 }} />
                              </button>
                            )}
                            <button type="button" className="btn btn-outline-light" onClick={() => toggleSvcDrop(row.id)}>
                              <i className={`ti ti-chevron-${svcOpenId === row.id ? "up" : "down"}`} />
                            </button>
                          </div>
                        </div>
                        {/* Quantity */}
                        <div style={{ width: 80, flexShrink: 0 }}>
                          <input
                            type="number"
                            className={`form-control text-center${errors[`svcQty_${row.id}`] ? " is-invalid" : ""}`}
                            min={0.0001}
                            step="any"
                            value={row.quantity}
                            onChange={(e) => { updateServiceRow(row.id, "quantity", e.target.value); clr(`svcQty_${row.id}`); }}
                            onBlur={(e) => {
                              const v = parseFloat(e.target.value);
                              if (!e.target.value.trim() || isNaN(v) || v <= 0)
                                updateServiceRow(row.id, "quantity", "1");
                            }}
                          />
                          {errors[`svcQty_${row.id}`] && (
                            <div className="text-danger fs-12 mt-1">{errors[`svcQty_${row.id}`]}</div>
                          )}
                        </div>
                        {/* Selling Price — read only */}
                        <div style={{ flex: 2 }}>
                        ...
                        <div className="d-flex gap-2 align-items-center" style={{ width: 40, flexShrink: 0 }}>
                          <button
                            ...
                            className="btn p-0 border-0 bg-transparent text-danger"
                            title="Remove"
                            disabled={associatedServiceRows.length === 1}
                    ...
                    <div className="px-3 pt-2 pb-1" style={{ background: "#fafafa" }}>
                      ...
                      <div className="d-flex align-items-center pb-2" style={{ gap: 12 }}>
                        ...
                        <div style={{ width: 80, flexShrink: 0 }} />
                        <div style={{ flex: 2, minWidth: 0 }}>
                          <span className="fs-14 fw-semibold">{svcTotalSelling.toFixed(2)}</span>
                        </div>
                        <div style={{ flex: 2, minWidth: 0 }}>
                          <span className="fs-14 fw-semibold">{svcTotalCost.toFixed(2)}</span>
                        </div>
                        <div style={{ width: 40, flexShrink: 0 }} />
                      </div>
                    </div>
                        ...
                        <div className="col-sm-4">
                          ...
                          {assocTotalSelling > 0 && (
                            <button
                              ...
                              style={{ textDecoration: "none" }}
                              onClick={() => { setSellingPrice(assocTotalSelling.toFixed(2)); clr("sellingPrice"); }}
                            >
                              Copy from total
                            </button>
                          )}
              ...
              {/* ══ Purchase Information (Assembly only) ════════════ */}
              {compositeType !== "kit" && <div className="border-top pt-4 mb-4">
                <div
                  className="d-flex align-items-center gap-2 mb-3"
                  style={{ cursor: "pointer" }}
                        ...
                        <div className="col-sm-4">
                          ...
                          {assocTotalCost > 0 && (
                            <button
                              ...
                              className="btn btn-link p-0 fs-12 text-primary mt-1"
                              style={{ textDecoration: "none" }}
                              onClick={() => { setCostPrice(assocTotalCost.toFixed(2)); clr("costPrice"); }}
                            >
                              Copy from total
                            </button>
                          )}
                      ...
                      <div className="mt-3">
                        ...
                        <div className="border rounded overflow-hidden">
                          ...
                          <div className="d-flex align-items-start px-3 py-2 border-bottom" style={{ background: "#f8f9fa", gap: 12 }}>
                            ...
                            <div style={{ flex: 2 }}>
                              <span className="fw-semibold fs-12 text-uppercase">Cost Price (₹) *</span>
                              <div>
                            ...
                            <div style={{ flex: 2 }}>
                              <span className="fw-semibold fs-12 text-uppercase">Selling Price (₹) *</span>
                              <div>
              ...
              {/* ══ Track Inventory (Goods + Assembly only) ══════════ */}
              {itemType === "goods" && compositeType !== "kit" && (
                <div className="border-top pt-4 mb-4">
                  <div
      ...
      {/* ── Associate Items search dropdown (portal — position:absolute with document coords) ── */}
      {assocOpenId !== null && assocDropPos !== null && ReactDOM.createPortal(
        <div
          ref={assocPortalRef}
          className="bg-white border rounded shadow-sm"
          style={{
            position: "absolute",
            top:  assocDropPos.top,
            left: assocDropPos.left,
            width: assocDropPos.width,
            zIndex: 9999,
            minWidth: 220,
          }}
        >
          <div className="p-2 border-bottom">
            <input
              ...
              value={assocSearchText[assocOpenId] ?? ""}
              onChange={(e) => {
                const v = e.target.value;
                setAssocSearchText((s) => ({ ...s, [assocOpenId!]: v }));
                if (assocSearchTimer.current) clearTimeout(assocSearchTimer.current);
                assocSearchTimer.current = setTimeout(() => loadAssocDropItems(v), 300);
              }}
            />
          </div>
          <div style={{ maxHeight: 200, overflowY: "auto" }}>
            ...
            ) : assocDropItems.length === 0 ? (
              <p className="text-muted fs-13 text-center py-2 mb-0">No results</p>
            ) : (
              assocDropItems.map((item) => (
                <div
                  key={item.id}
                  className="px-3 py-2 fs-15"
                  ...
                  onClick={() => {
                    ...
                    updateAssocRow(rowId, "item_name", item.name);
                    setAssocSearchText((s) => ({ ...s, [rowId]: item.name }));
                    setAssocImgErrors((s) => { const n = { ...s }; delete n[rowId]; return n; });
                    setAssocOpenId(null);
                    setAssocDropPos(null);
                    fetchItem(item.id).then((res) => {
                      if (res.success) {
                        ...
                        updateAssocRow(rowId, "item_image",    toStorageUrl(d.image as string | null));
                        updateAssocRow(rowId, "selling_price", d.selling_price != null ? String(d.selling_price) : (item.selling_price ?? "0.00"));
                        updateAssocRow(rowId, "cost_price",    d.cost_price    != null ? String(d.cost_price)    : (item.cost_price    ?? "0.00"));
                      } else {
                        updateAssocRow(rowId, "item_image",    toStorageUrl(item.image));
                        updateAssocRow(rowId, "selling_price", item.selling_price ?? "0.00");
                        updateAssocRow(rowId, "cost_price",    item.cost_price    ?? "0.00");
                      }
                    });
                  }}
                >
          ...
          <div className="border-top px-3 py-2">
            <button
              ...
              className="btn btn-link p-0 fs-14 text-primary fw-medium"
              style={{ textDecoration: "none" }}
              onClick={() => { setAssocOpenId(null); setAssocDropPos(null); }}
            >
              + Add New Item
            </button>
          </div>
      ...
      {/* ── Associate Services search dropdown (portal — position:absolute with document coords) ── */}
      {svcOpenId !== null && svcDropPos !== null && ReactDOM.createPortal(
        <div
          ref={svcPortalRef}
          className="bg-white border rounded shadow-sm"
          style={{
            position: "absolute",
            top:  svcDropPos.top,
            left: svcDropPos.left,
            width: svcDropPos.width,
            zIndex: 9999,
            minWidth: 220,
          }}
        >
          <div className="p-2 border-bottom">
            <input
              ...
              style={{ height: 42 }}
              placeholder="Search Items…"
              value={svcSearchText[svcOpenId] ?? ""}
              onChange={(e) => {
                const v = e.target.value;
                setSvcSearchText((s) => ({ ...s, [svcOpenId!]: v }));
                if (svcSearchTimer.current) clearTimeout(svcSearchTimer.current);
                svcSearchTimer.current = setTimeout(() => loadSvcDropItems(v), 300);
              }}
            />
          </div>
          <div style={{ maxHeight: 200, overflowY: "auto" }}>
            {svcDropLoading ? (
              <p className="text-muted fs-13 text-center py-2 mb-0">Loading…</p>
            ) : svcDropItems.length === 0 ? (
              <p className="text-muted fs-13 text-center py-2 mb-0">No results</p>
            ) : (
              svcDropItems.map((item) => (
                <div
                  key={item.id}
                  className="px-3 py-2 fs-15"
                  style={{ cursor: "pointer", color: "#707070" }}
                  ...
                  onClick={() => {
                    ...
                    updateServiceRow(rowId, "item_name", item.name);
                    setSvcSearchText((s) => ({ ...s, [rowId]: item.name }));
                    setSvcImgErrors((s) => { const n = { ...s }; delete n[rowId]; return n; });
                    setSvcOpenId(null);
                    setSvcDropPos(null);
                    fetchItem(item.id).then((res) => {
                      if (res.success) {
                        const d = res.data as Record<string, unknown>;
                        updateServiceRow(rowId, "item_image",    toStorageUrl(d.image as string | null));
                        updateServiceRow(rowId, "selling_price", d.selling_price != null ? String(d.selling_price) : (item.selling_price ?? "0.00"));
                        updateServiceRow(rowId, "cost_price",    d.cost_price    != null ? String(d.cost_price)    : (item.cost_price    ?? "0.00"));
                      } else {
                        updateServiceRow(rowId, "item_image",    toStorageUrl(item.image));
                        updateServiceRow(rowId, "selling_price", item.selling_price ?? "0.00");
                        updateServiceRow(rowId, "cost_price",    item.cost_price    ?? "0.00");
                      }
                    });
                  }}
                >
          ...
          <div className="border-top px-3 py-2">
            <button
              ...
              className="btn btn-link p-0 fs-14 text-primary fw-medium"
              style={{ textDecoration: "none" }}
              onClick={() => { setSvcOpenId(null); setSvcDropPos(null); }}
            >
              + Add New Item
            </button>
          </div>
