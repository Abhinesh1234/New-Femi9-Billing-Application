...
function isRelativeDateKey(v: string): boolean {
  return relativeDateOptions.some((o) => o.value === v);
}
...
const EditCustomField = () => {
  ...
  const fieldIdNum = id ? parseInt(id, 10) : NaN;
  ...
  const [multiselectOptions, setMultiselectOptions] = useState<MultiselectOpt[]>([
  ...
  // Data types that cannot appear in transactions
  ...
  // ── Fetch and populate ────────────────────────────────────────────────────
  const populateFromConfig = (field: CustomField) => {
    const c = field.config;
    const tc = (c.type_config ?? {}) as Record<string, unknown>;

    setOriginalFieldKey(c.field_key);
    setOriginalIsSystem(c.is_system);
    setLabel(c.label);
    setDataType(c.data_type);
    setHelpText(c.help_text ?? "");
    setMandatory(c.is_mandatory);
    setIsActive(c.is_active);
    setShowInTx(c.show_in_transactions);
    setShowInPdfs(c.show_in_all_pdfs);
    setIncludeModules(c.include_in_modules ?? []);

    // Privacy
    setPii(c.privacy?.is_pii ?? false);
    setEphi(c.privacy?.is_ephi ?? false);
    setEncryptData(c.privacy?.encrypt_data ?? true);

    // Type-config
    if (c.data_type === "text_single" || c.data_type === "text_multi") {
      const fmt = tc.input_format as string | null;
      const customFmt = tc.custom_input_format as string | null;
      if (customFmt) {
        setCustomFormat(true);
        setCustomFormatText(customFmt);
      } else if (fmt) {
        setInputFormat(fmt);
      }
      if (c.data_type === "text_multi") {
        setRichTextEditor(!!(tc.rich_text_editor));
      }
    } else if (c.data_type === "url") {
      setHyperlinkLabel((tc.hyperlink_label as string) ?? "");
    } else if (c.data_type === "auto_generate") {
      setAutoPrefix((tc.prefix as string) ?? "");
      setAutoStartingNumber(String(tc.starting_number ?? ""));
      setAutoSuffix((tc.suffix as string) ?? "");
    } else if (c.data_type === "dropdown") {
      const opts = (tc.options as Array<{ id: number; label: string; color: string; is_active: boolean; sort_order: number }>) ?? [];
      const mapped: DropdownOpt[] = opts
        .sort((a, b) => a.sort_order - b.sort_order)
        .map((o) => ({ id: o.id, label: o.label, color: o.color ?? "#e53935", active: o.is_active }));
      setDropdownOptions(mapped.length > 0 ? mapped : [
        { id: 1, label: "", color: "#e53935", active: true },
        { id: 2, label: "", color: "#1e88e5", active: true },
        { id: 3, label: "", color: "#fb8c00", active: true },
        { id: 4, label: "", color: "#43a047", active: true },
      ...
      if (mapped.length > 0) {
        dropdownNextId.current = Math.max(...mapped.map((o) => o.id)) + 1;
      }
      setDropdownAddColor(!!(tc.add_color));
      setDropdownColorPlacement((tc.color_placement as "next" | "wrap") ?? "next");
    } else if (c.data_type === "multiselect") {
      const opts = (tc.options as Array<{ id: number; label: string; is_active: boolean; sort_order: number }>) ?? [];
      const mapped: MultiselectOpt[] = opts
        .sort((a, b) => a.sort_order - b.sort_order)
        .map((o) => ({ id: o.id, label: o.label, active: o.is_active }));
      setMultiselectOptions(mapped.length > 0 ? mapped : [
        { id: 1, label: "", active: true },
        { id: 2, label: "", active: true },
        { id: 3, label: "", active: true },
        { id: 4, label: "", active: true },
      ...
      if (mapped.length > 0) {
        multiselectNextId.current = Math.max(...mapped.map((o) => o.id)) + 1;
      }
    } else if (c.data_type === "attachment") {
      const types = (tc.allowed_file_types as string[]) ?? [];
      setAttachmentFileImage(types.includes("image"));
      setAttachmentFileDocument(types.includes("document"));
      setAttachmentFilePDF(types.includes("pdf"));
      setAttachmentFileAllFiles(types.includes("all_files"));
    } else if (c.data_type === "lookup") {
      setLookupModule((tc.lookup_module as string) ?? "");
    }

    // Default value
    const dv = c.default_value;
    if (c.data_type === "date" && dv) {
      if (isRelativeDateKey(dv)) {
        setDateDefaultValue(dv);
      } else {
        setDateCustomMode(true);
        setDateCustomValue(dayjs(dv, "YYYY-MM-DD"));
      }
    } else if (c.data_type === "datetime" && dv) {
      setDatetimeDefaultValue(dayjs(dv));
    } else if (c.data_type === "checkbox") {
      setCheckboxDefault(dv === "1");
    } else if (c.data_type === "dropdown") {
      setDropdownDefault(dv ?? "");
    } else if (c.data_type === "multiselect") {
      setMultiselectDefault(dv ?? "");
    } else {
      setDefaultValue(dv ?? "");
    }
  };
  ...
  useEffect(() => {
    if (isNaN(fieldIdNum)) {
      setFetchError("Invalid field ID.");
      setLoading(false);
      return;
    }
  ...
  const updateDropdownOptionLabel = (id: number, lbl: string) => {
  ...
  const buildConfig = (): CustomFieldConfig => {
    ...
    if (dataType === "text_single") {
      ...
      type_config = { prefix: autoPrefix || null, starting_number: parseInt(autoStartingNumber, 10), suffix: autoSuffix || null };
    ...
    return (
      <div className="page-wrapper">
        <div className="content">
          ...
          <button className="btn btn-sm btn-cancel" onClick={goBack}>Go Back</button>
        </div>
        <Footer />
      </div>
    );
                  ...
                  <div className="row mb-3 align-items-start">
                    ...
                    <div className="col-sm-8">
                      <CommonSelect
                        ...
                        isDisabled
                      />
                      <small className="text-muted d-block mt-1">Data type cannot be changed after creation.</small>
                    </div>
                  </div>
                    ...
                    <>
                          ...
                          <div className="col-sm-8 offset-sm-4">
                            <div className="form-check mb-0">
                              <input
                                ...
                                type="checkbox"
                                id="edit_rich_text_editor"
                                checked={richTextEditor}
                                onChange={(e) => setRichTextEditor(e.target.checked)}
                              />
                              <label className="form-check-label fs-14" htmlFor="edit_rich_text_editor">
                        ...
                        <div className="col-sm-8">
                          <div className="d-flex align-items-center gap-3 mb-2">
                            <div className="form-check mb-0">
                              ...
                              />
                              <label className="form-check-label" htmlFor="edit_pii">PII</label>
                            </div>
                            <div className="form-check mb-0">
                              <input
                                className="form-check-input"
                              ...
                              />
                              <label className="form-check-label" htmlFor="edit_ephi">ePHI</label>
                            </div>
                          </div>
                          ...
                          {pii && !ephi && (
                            <div>
                              <div className="form-check mb-2">
                                ...
                                />
                                <label className="form-check-label fw-medium" htmlFor="edit_encrypt_yes">
                              ...
                              <div className="form-check mb-0">
                                ...
                                />
                                <label className="form-check-label fw-medium" htmlFor="edit_encrypt_no">
                        ...
                        <div className="row mb-3 align-items-start">
                          ...
                          <div className="col-sm-8">
                            <div className="row g-2">
                              <div className="col-6">
                                <div className="form-check mb-0">
                                  <input className="form-check-input" type="checkbox" id="edit_att_image"
                                    checked={attachmentFileImage} onChange={(e) => { setAttachmentFileImage(e.target.checked); setErrors((p) => ({ ...p, allowed_file_types: "" })); }} />
                                  <label className="form-check-label" htmlFor="edit_att_image">Image</label>
                                </div>
                              </div>
                              <div className="col-6">
                                <div className="form-check mb-0">
                                  <input className="form-check-input" type="checkbox" id="edit_att_document"
                                    checked={attachmentFileDocument} onChange={(e) => { setAttachmentFileDocument(e.target.checked); setErrors((p) => ({ ...p, allowed_file_types: "" })); }} />
                                  <label className="form-check-label" htmlFor="edit_att_document">Document</label>
                                </div>
                              </div>
                              <div className="col-6">
                                <div className="form-check mb-0">
                                  <input className="form-check-input" type="checkbox" id="edit_att_pdf"
                                    checked={attachmentFilePDF} onChange={(e) => { setAttachmentFilePDF(e.target.checked); setErrors((p) => ({ ...p, allowed_file_types: "" })); }} />
                                  <label className="form-check-label" htmlFor="edit_att_pdf">PDF</label>
                                </div>
                              </div>
                              <div className="col-6">
                                <div className="form-check mb-0">
                                  <input className="form-check-input" type="checkbox" id="edit_att_all"
                                    checked={attachmentFileAllFiles} onChange={(e) => { setAttachmentFileAllFiles(e.target.checked); setErrors((p) => ({ ...p, allowed_file_types: "" })); }} />
                                  <label className="form-check-label" htmlFor="edit_att_all">All Files</label>
                                </div>
                              </div>
                            </div>
                                    ...
                                    <div className="d-flex align-items-center gap-3">
                                      <div className="form-check mb-0">
                                        <input
                                          className="form-check-input"
                                          type="checkbox"
                                          id="edit_dd_add_color"
                                          checked={dropdownAddColor}
                                          onChange={(e) => setDropdownAddColor(e.target.checked)}
                                        />
                                        <label className="form-check-label fs-13" htmlFor="edit_dd_add_color">Add color to options</label>
                                      </div>
                                      <button
                                        type="button"
                                ...
                                <div className="d-flex align-items-center gap-4">
                                  <div className="form-check mb-0 d-flex align-items-center gap-2">
                                    <input
                                      className="form-check-input"
                                      type="radio"
                                      name="edit_dd_color_placement"
                                      id="edit_dd_cp_next"
                                      checked={dropdownColorPlacement === "next"}
                                      onChange={() => setDropdownColorPlacement("next")}
                                    />
                                    <label className="form-check-label d-flex align-items-center gap-1" htmlFor="edit_dd_cp_next">
                                  ...
                                  <div className="form-check mb-0 d-flex align-items-center gap-2">
                                    ...
                                    />
                                    <label className="form-check-label" htmlFor="edit_dd_cp_wrap">
                          ...
                          <div className="col-sm-8">
                            {(dataType === "attachment" || dataType === "image") ? (
                              ...
                              <div className="input-group">
                                ...
                                <input type="email" className="form-control border-start-0" value={defaultValue} onChange={(e) => setDefaultValue(e.target.value)} placeholder="Enter default email" />
                              </div>
                            ) : dataType === "url" ? (
                              <div className="input-group">
                                <span className="input-group-text bg-white"><i className="ti ti-world text-muted" /></span>
                                <input type="url" className="form-control border-start-0" value={defaultValue} onChange={(e) => setDefaultValue(e.target.value)} placeholder="Enter default URL" />
                              </div>
                            ) : dataType === "phone" ? (
                              <div className="input-group">
                                <span className="input-group-text bg-white"><i className="ti ti-phone text-muted" /></span>
                                <input type="tel" className="form-control border-start-0" value={defaultValue} onChange={(e) => setDefaultValue(e.target.value)} placeholder="Enter default phone" />
                              </div>
                            ) : dataType === "amount" ? (
                              <div className="input-group">
                                <span className="input-group-text bg-white">INR</span>
                                <input type="number" className="form-control border-start-0" value={defaultValue} onChange={(e) => setDefaultValue(e.target.value)} placeholder="0.00" />
                              </div>
                            ) : dataType === "percent" ? (
                              <div className="input-group">
                                <input type="number" className="form-control border-end-0" value={defaultValue} onChange={(e) => setDefaultValue(e.target.value)} />
                                <span className="input-group-text bg-white">%</span>
                              </div>
                            ) : dataType === "dropdown" ? (
                              <select className="form-select" value={dropdownDefault} onChange={(e) => setDropdownDefault(e.target.value)}>
                                <option value=""></option>
                              ...
                              <select className="form-select" value={multiselectDefault} onChange={(e) => setMultiselectDefault(e.target.value)}>
                                <option value=""></option>
                              ...
                              <div className="form-check mb-0">
                                <input className="form-check-input" type="checkbox" id="edit_checkbox_default"
                                  checked={checkboxDefault} onChange={(e) => setCheckboxDefault(e.target.checked)} />
                                <label className="form-check-label" htmlFor="edit_checkbox_default">Ticked by default</label>
                              </div>
                            ) : dataType === "datetime" ? (
                              <CommonDatePicker
                                            ...
                                            .map((o) => {
                                              ...
                                              return (
                                                <div
                                                  key={o.value}
                                                  className={`input-format-option px-3 py-2${selected ? " is-selected" : ""}`}
                                                  onClick={() => { setDateDefaultValue(o.value); setDateDefaultOpen(false); setDateDefaultSearch(""); }}
                                                >
                                                  {o.label}
                                                </div>
                                              );
                              ...
                              <textarea className="form-control" rows={3} value={defaultValue} onChange={(e) => setDefaultValue(e.target.value)} />
                            ) : (
                              <input type="text" className="form-control" value={defaultValue} onChange={(e) => setDefaultValue(e.target.value)} />
                            )}
                          </div>
                        </div>
                      )}
                      ...
                      {supportsShowInTx ? (
                        <div className="row mb-3 align-items-center">
                          ...
                          <div className="col-sm-8">
                            <div className="d-flex align-items-center gap-3">
                              <div className="form-check mb-0">
                                <input className="form-check-input" type="radio" name="edit_show_tx" id="edit_show_tx_yes"
                                  checked={showInTx === true} onChange={() => setShowInTx(true)} />
                                <label className="form-check-label" htmlFor="edit_show_tx_yes">Yes</label>
                              </div>
                              <div className="form-check mb-0">
                                <input className="form-check-input" type="radio" name="edit_show_tx" id="edit_show_tx_no"
                                  checked={showInTx === false} onChange={() => setShowInTx(false)} />
                                <label className="form-check-label" htmlFor="edit_show_tx_no">No</label>
                              </div>
                            </div>
                          </div>
                        </div>
                              ...
                              {modulesOpen && (
                                ...
                                >
                                  ...
                                  <div className="px-3 py-2 border-bottom">
                                    <div className="form-check mb-0">
                                      <input className="form-check-input" type="checkbox" id="edit_mod_all"
                                      ...
                                      <label className="form-check-label fw-medium" htmlFor="edit_mod_all">All</label>
                                    </div>
                                  </div>
                                  <div style={{ maxHeight: 260, overflowY: "auto" }}>
                                        ...
                                        <div key={group}>
                                          ...
                                          {filtered.map((o) => (
                                            <div key={o.value} className="px-3 py-2">
                                              <div className="form-check mb-0">
                                                <input className="form-check-input" type="checkbox" id={`edit_mod_${o.value}`}
                                                  checked={includeModules.includes(o.value)}
                                                  onChange={(e) => setIncludeModules((prev) => e.target.checked ? [...prev, o.value] : prev.filter((v) => v !== o.value))} />
                                                <label className="form-check-label" htmlFor={`edit_mod_${o.value}`}>{o.label}</label>
                                              </div>
                                            </div>
                                          ))}
                                        </div>
                      ...
                      {dataType !== "auto_generate" && dataType !== "text_multi" && dataType !== "dropdown" && dataType !== "multiselect" && dataType !== "lookup" && dataType !== "attachment" && dataType !== "image" && <div className="row mb-3 align-items-center">
                        ...
                        <div className="col-sm-8">
                          <div className="d-flex align-items-center gap-3">
                            <div className="form-check mb-0">
                              <input className="form-check-input" type="radio" name="edit_show_pdfs" id="edit_show_pdfs_yes"
                                checked={showInPdfs === true} onChange={() => setShowInPdfs(true)} />
                              <label className="form-check-label" htmlFor="edit_show_pdfs_yes">Yes</label>
                            </div>
                            <div className="form-check mb-0">
                              <input className="form-check-input" type="radio" name="edit_show_pdfs" id="edit_show_pdfs_no"
                                checked={showInPdfs === false} onChange={() => setShowInPdfs(false)} />
                              <label className="form-check-label" htmlFor="edit_show_pdfs_no">No</label>
                            </div>
                          </div>
                        </div>
                      </div>}
                      ...
                      <div className="row mb-3 align-items-center">
                        ...
                        <div className="col-sm-8">
                          <div className="d-flex align-items-center gap-3">
                            <div className="form-check mb-0">
                              <input className="form-check-input" type="radio" name="edit_mandatory" id="edit_mandatory_yes"
                                checked={mandatory === true} onChange={() => setMandatory(true)} />
                              <label className="form-check-label" htmlFor="edit_mandatory_yes">Yes</label>
                            </div>
                            <div className="form-check mb-0">
                              <input className="form-check-input" type="radio" name="edit_mandatory" id="edit_mandatory_no"
                                checked={mandatory === false} onChange={() => setMandatory(false)} />
                              <label className="form-check-label" htmlFor="edit_mandatory_no">No</label>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="row mb-3 align-items-center">
                        ...
                        <div className="col-sm-8">
                          <div className="d-flex align-items-center gap-3">
                            <div className="form-check mb-0">
                              <input className="form-check-input" type="radio" name="edit_active" id="edit_active_yes"
                                checked={isActive === true} onChange={() => setIsActive(true)} />
                              <label className="form-check-label" htmlFor="edit_active_yes">Active</label>
                            </div>
                            <div className="form-check mb-0">
                              <input className="form-check-input" type="radio" name="edit_active" id="edit_active_no"
                                checked={isActive === false} onChange={() => setIsActive(false)} />
                              <label className="form-check-label" htmlFor="edit_active_no">Inactive</label>
                            </div>
                          </div>
                        </div>
                      </div>
