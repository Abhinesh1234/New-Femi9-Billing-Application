...
const inputFormatOptions = [
  { value: "numbers",                          label: "Numbers",                                              desc: "Accepts only numbers 0-9." },
  { value: "alphanumeric_no_spaces",           label: "Alphanumeric Characters Without Spaces",              desc: "Accepts a-z, A-Z, 0-9." },
  { value: "alphanumeric_with_spaces",         label: "Alphanumeric Characters With Spaces",                 desc: "Accepts a-z, A-Z, 0-9, and spaces." },
  { value: "alphanumeric_hyphens_underscores", label: "Alphanumeric Characters With Hyphens and Underscores", desc: "Accepts a-z, A-Z, 0-9, hyphens, and underscores." },
  { value: "alphabets_no_spaces",              label: "Alphabets Without Spaces",                            desc: "Accepts only a-z and A-Z." },
  { value: "alphabets_with_spaces",            label: "Alphabets With Spaces",                               desc: "Accepts a-z, A-Z, and spaces." },
];
...
const AddCustomField = () => {
  ...
  // F16 — read module from URL, default to "products"
  const module = VALID_MODULES.includes(searchParams.get("module") ?? "")
  ...
  useEffect(() => {
    const handler = (e: MouseEvent | TouchEvent) => {
      if (inputFormatRef.current    && !inputFormatRef.current.contains(e.target as Node))    setInputFormatOpen(false);
      if (modulesRef.current        && !modulesRef.current.contains(e.target as Node))        setModulesOpen(false);
      if (dateDefaultRef.current    && !dateDefaultRef.current.contains(e.target as Node))    setDateDefaultOpen(false);
      ...
      if (dropdownMoreMenuRef.current   && !dropdownMoreMenuRef.current.contains(e.target as Node))   setDropdownMoreMenuOpen(null);
      if (multiselectMoreMenuRef.current && !multiselectMoreMenuRef.current.contains(e.target as Node)) setMultiselectMoreMenuOpen(null);
      if (lookupModuleRef.current   && !lookupModuleRef.current.contains(e.target as Node))   setLookupModuleOpen(false);
    };
  ...
  const [fieldKey, setFieldKey]                   = useState("");      // F14 — visible/editable
  const [fieldKeyManual, setFieldKeyManual]       = useState(false);   // F14
  ...
  // F14 — auto-generate fieldKey from label when not manually overridden
  useEffect(() => {
    if (!fieldKeyManual) {
      setFieldKey(labelToFieldKey(label));
    }
  }, [label, fieldKeyManual]);

  // F12 — warn before unload when the form is dirty
  useEffect(() => {
  ...
  // F13 — simplified: hasConditionalFields is true whenever a data type is selected
  const hasConditionalFields = dataType !== "";
  ...
  const addDropdownOptionAfter = useCallback((id: number) => {
    setDropdownOptions((prev) => {
  ...
  const updateDropdownOptionLabel = useCallback((id: number, lbl: string) => {
    setDropdownOptions((prev) => prev.map((o) => (o.id === id ? { ...o, label: lbl } : o)));
    setIsDirty(true);
  }, []);

  const updateDropdownColor = useCallback((id: number, color: string) => {
    setDropdownOptions((prev) => prev.map((o) => (o.id === id ? { ...o, color } : o)));
    setIsDirty(true);
  }, []);

  const toggleDropdownOptionActive = useCallback((id: number) => {
    setDropdownOptions((prev) => prev.map((o) => (o.id === id ? { ...o, active: !o.active } : o)));
    setIsDirty(true);
  }, []);

  const deleteDropdownOption = useCallback((id: number) => {
    setDropdownOptions((prev) => prev.filter((o) => o.id !== id));
    setIsDirty(true);
  }, []);

  const handleDropdownDragEnd = useCallback((event: DragEndEvent) => {
  ...
  // ── Multiselect option handlers ───────────────────────────────────────────
  const addMultiselectOption = useCallback(() => {
    setMultiselectOptions((prev) => [...prev, { id: multiselectNextId.current++, label: "", active: true }]);
    setIsDirty(true);
  }, []);

  const addMultiselectOptionAfter = useCallback((id: number) => {
  ...
  const updateMultiselectOptionLabel = useCallback((id: number, lbl: string) => {
    setMultiselectOptions((prev) => prev.map((o) => (o.id === id ? { ...o, label: lbl } : o)));
    setIsDirty(true);
  }, []);

  const toggleMultiselectOptionActive = useCallback((id: number) => {
    setMultiselectOptions((prev) => prev.map((o) => (o.id === id ? { ...o, active: !o.active } : o)));
    setIsDirty(true);
  }, []);

  const deleteMultiselectOption = useCallback((id: number) => {
    setMultiselectOptions((prev) => prev.filter((o) => o.id !== id));
    setIsDirty(true);
  }, []);

  const handleMultiselectDragEnd = useCallback((event: DragEndEvent) => {
    const { active, over } = event;
    if (over && active.id !== over.id) {
      setMultiselectOptions((prev) => {
        const oldIndex = prev.findIndex((o) => o.id === active.id);
  ...
  const buildConfig = useCallback((): CustomFieldConfig => {
    ...
    if (dataType === "text_single") {
      ...
      // F7 — strip frontend-only `id` from options
      type_config = {
        add_color: dropdownAddColor,
        color_placement: dropdownColorPlacement,
        options: dropdownOptions.map((o, i) => ({ label: o.label, color: o.color, is_active: o.active, sort_order: i + 1 })),
      };
    } else if (dataType === "multiselect") {
      // F7 — strip frontend-only `id` from options
      type_config = {
        options: multiselectOptions.map((o, i) => ({ label: o.label, is_active: o.active, sort_order: i + 1 })),
      };
    ...
    return {
      label:                label.trim(),
      field_key:            fieldKey || labelToFieldKey(label),
      data_type:            dataType,
    ...
    dataType, label, fieldKey, mandatory, helpText, showInTx, showInPdfs,
    includeModules, pii, ephi, encryptData, inputFormat, customFormat, customFormatText,
    richTextEditor, hyperlinkLabel, autoPrefix, autoStartingNumber, autoSuffix, autoAddToExisting,
    dropdownAddColor, dropdownColorPlacement, dropdownOptions, dropdownDefault,
    multiselectOptions, multiselectDefault, attachmentFileImage, attachmentFileDocument,
    attachmentFilePDF, attachmentFileAllFiles, lookupModule, dateCustomMode, dateCustomValue,
    dateDefaultValue, datetimeDefaultValue, checkboxDefault, defaultValue,
  ]);

  // ── Frontend validation ───────────────────────────────────────────────────
  const validate = useCallback((config: CustomFieldConfig): Record<string, string> => {
    ...
    // F6 — validate auto_generate starting number is a positive integer ≥ 1
    if (config.data_type === "auto_generate") {
      const n = parseInt(autoStartingNumber, 10);
      if (!autoStartingNumber || isNaN(n) || n < 1 || !Number.isInteger(n)) {
        errs.starting_number = "Starting Number must be a positive whole number (e.g. 1, 100).";
      }
    }
    ...
    // F8 — validate dropdown/multiselect options have non-empty labels
    if (config.data_type === "dropdown") {
      const hasEmpty = dropdownOptions.some((o) => o.active && !o.label.trim());
      if (hasEmpty) errs.dropdown_options = "All active dropdown options must have a label.";
    }
    if (config.data_type === "multiselect") {
      const hasEmpty = multiselectOptions.some((o) => o.active && !o.label.trim());
      if (hasEmpty) errs.multiselect_options = "All active multiselect options must have a label.";
    }
    ...
    return errs;
  }, [lookupModule, autoStartingNumber, dropdownOptions, multiselectOptions]);
    ...
    <>
      <div className="page-wrapper">
        <div className="content">
          <PageHeader
            title={`New Custom Field — ${MODULE_LABELS[module] ?? module}`}
            badgeCount={false}
            ...
            <div className="col-12">
              <div className="card mb-0">
                <div className="card-body p-4">
                  ...
                  {/* F14 — Field Key (visible + editable) */}
                  <div className="row mb-3 align-items-center">
                    <label className="col-sm-4 col-form-label fw-medium fs-14 d-flex align-items-center gap-1">
                      Field Key (API name)
                      <OverlayTrigger placement="right" overlay={<Tooltip>The unique identifier used when accessing this field via the API. Auto-generated from the label.</Tooltip>}>
                    ...
                    <div className="col-sm-8">
                      <div className="input-group">
                        <input
                          type="text"
                          className={`form-control font-monospace ${errors.field_key ? "is-invalid" : ""}`}
                          value={fieldKey}
                          onChange={(e) => {
                            setFieldKey(e.target.value);
                            setFieldKeyManual(true);
                            setIsDirty(true);
                            setErrors((p) => ({ ...p, field_key: "" }));
                          }}
                        ...
                        {fieldKeyManual && (
                          <button
                            ...
                            onClick={() => { setFieldKeyManual(false); setFieldKey(labelToFieldKey(label)); }}
                          >
                            <i className="ti ti-refresh" />
                          </button>
                        )}
                      </div>
                      {errors.field_key && <div className="invalid-feedback d-block">{errors.field_key}</div>}
                      <small className="text-muted">
                        {fieldKeyManual ? "Custom key — must start with a letter, lowercase letters/numbers/underscores only." : "Auto-generated from label."}
                      </small>
                    </div>
                  </div>
                  ...
                  {/* Conditional fields — shown once a data type is selected */}
                  {hasConditionalFields && (
                    <>
                      {/* Rich-text editor — text_multi only */}
                      {dataType === "text_multi" && (
                        <div className="row mb-3 align-items-center">
                          <div className="col-sm-8 offset-sm-4">
                            <div className="form-check mb-0">
                              <input
                                ...
                                onChange={(e) => { setRichTextEditor(e.target.checked); setIsDirty(true); }}
                              />
                        ...
                        <div className="row mb-3 align-items-center">
                          ...
                          <div className="col-sm-8">
                            <input
                              ...
                              onChange={(e) => { setHyperlinkLabel(e.target.value); setIsDirty(true); }}
                            />
                          </div>
                        </div>
                      )}
                      ...
                      <div className="row mb-3 align-items-start">
                        ...
                        <div className="col-sm-8">
                          <input
                            ...
                            className="form-control"
                            value={helpText}
                            onChange={(e) => { setHelpText(e.target.value); setIsDirty(true); }}
                          />
                        ...
                        <div className="row mb-3 align-items-start">
                          ...
                          <div className="col-sm-8">
                            <div style={{ position: "relative" }} ref={lookupModuleRef}>
                              ...
                              {lookupModuleOpen && (
                                <div className="position-absolute w-100 bg-white border rounded shadow" style={{ zIndex: 1050, top: "calc(100% + 2px)", left: 0 }}>
                                  <div className="p-2 border-bottom">
                                    <div className="input-group">
                                      <span className="input-group-text bg-white border-end-0"><i className="ti ti-search text-muted" /></span>
                                      <input autoFocus type="text" className="form-control border-start-0 ps-0" placeholder="Search"
                                        value={lookupModuleSearch} onChange={(e) => setLookupModuleSearch(e.target.value)} />
                                    </div>
                                  </div>
                                  <div style={{ maxHeight: 240, overflowY: "auto" }}>
                                    {lookupModuleOptions.filter((m) => m.toLowerCase().includes(lookupModuleSearch.toLowerCase())).map((m) => (
                                      <div key={m}
                                        className={`input-format-option px-3 py-2${lookupModule === m ? " is-selected" : ""}`}
                                        onClick={() => { setLookupModule(m); setLookupModuleOpen(false); setLookupModuleSearch(""); setIsDirty(true); setErrors((p) => ({ ...p, lookup_module: "" })); }}>
                                        {m}
                                      </div>
                                    ))}
                                  </div>
                      ...
                      {/* Data Privacy */}
                      {!["number","decimal","amount","percent","checkbox","auto_generate","dropdown","multiselect","lookup","attachment","image"].includes(dataType) && (
                        <div className="row mb-3 align-items-start">
                          <label className="col-sm-4 col-form-label fw-medium fs-14">Data Privacy</label>
                          <div className="col-sm-8">
                            <div className="d-flex align-items-center gap-3 mb-2">
                              <div className="form-check mb-0">
                                <input className="form-check-input" type="checkbox" id="add_pii"
                                  checked={pii || ephi} disabled={ephi}
                                  onChange={(e) => { setPii(e.target.checked); setEncryptData(true); setIsDirty(true); }} />
                                <label className="form-check-label" htmlFor="add_pii">PII</label>
                              </div>
                              <div className="form-check mb-0">
                                <input className="form-check-input" type="checkbox" id="add_ephi"
                                  checked={ephi}
                                  onChange={(e) => { setEphi(e.target.checked); if (e.target.checked) setPii(true); setEncryptData(true); setIsDirty(true); }} />
                                <label className="form-check-label" htmlFor="add_ephi">ePHI</label>
                              </div>
                            </div>
                            {!pii && !ephi && <small className="text-muted d-block">Data will be stored without encryption and will be visible to all users.</small>}
                            {pii && !ephi && (
                              <div>
                                <div className="form-check mb-2">
                                  <input className="form-check-input" type="radio" name="add_encrypt" id="add_encrypt_yes"
                                    checked={encryptData} onChange={() => { setEncryptData(true); setIsDirty(true); }} />
                                  <label className="form-check-label fw-medium" htmlFor="add_encrypt_yes">Sensitive data. Encrypt and store it.</label>
                                  {encryptData && <small className="text-muted d-block mt-1">Only users with access to protected data will be able to view the details, and this field cannot be used to perform an advanced search.</small>}
                                </div>
                                <div className="form-check mb-0">
                                  <input className="form-check-input" type="radio" name="add_encrypt" id="add_encrypt_no"
                                    checked={!encryptData} onChange={() => { setEncryptData(false); setIsDirty(true); }} />
                                  <label className="form-check-label fw-medium" htmlFor="add_encrypt_no">Not sensitive data. Store it without encryption.</label>
                                  {!encryptData && <small className="text-muted d-block mt-1">Only users with access to protected data will be able to view the details, but this field can be used to perform an advanced search.</small>}
                                </div>
                              </div>
                            )}
                            {((pii && ephi) || (!pii && ephi)) && <small className="text-muted d-block">Data is sensitive and will be stored with encryption. Only users with access to protected data will be able to view it.</small>}
                          </div>
                        </div>
                      )}
                        ...
                        <div className="row mb-3 align-items-start">
                          ...
                          <div className="col-sm-8">
                            <div className="row g-2">
                              {[
                                { id: "att_image",    label: "Image",     state: attachmentFileImage,    setter: setAttachmentFileImage },
                                { id: "att_document", label: "Document",  state: attachmentFileDocument, setter: setAttachmentFileDocument },
                                { id: "att_pdf",      label: "PDF",       state: attachmentFilePDF,      setter: setAttachmentFilePDF },
                                { id: "att_all",      label: "All Files", state: attachmentFileAllFiles, setter: setAttachmentFileAllFiles },
                              ].map((item) => (
                                <div key={item.id} className="col-6">
                                  <div className="form-check mb-0">
                                    <input className="form-check-input" type="checkbox" id={item.id}
                                      checked={item.state}
                                      onChange={(e) => { item.setter(e.target.checked); setIsDirty(true); setErrors((p) => ({ ...p, allowed_file_types: "" })); }} />
                      ...
                      {/* Input Format */}
                      {!["date","datetime","checkbox","auto_generate","dropdown","multiselect","lookup","attachment","image"].includes(dataType) && (
                        <div className="row mb-3 align-items-start">
                          <label className="col-sm-4 col-form-label fw-medium fs-14">Input Format</label>
                          <div className="col-sm-8">
                            {["email","url","phone","number","decimal","amount","percent"].includes(dataType) ? (
                              <input type="text" className="form-control" value={inputFormat}
                                onChange={(e) => { setInputFormat(e.target.value); setIsDirty(true); }} />
                            ) : customFormat ? (
                              <div className="d-flex align-items-center gap-2">
                                <input type="text" className="form-control" value={customFormatText}
                                  onChange={(e) => { setCustomFormatText(e.target.value); setIsDirty(true); }} placeholder="Enter custom format" />
                                <Link to="#" className="text-primary text-nowrap fs-13"
                              ...
                              <div className="d-flex align-items-center gap-2">
                                <div className="position-relative flex-grow-1" ref={inputFormatRef}>
                                  ...
                                  {inputFormatOpen && (
                                    <div className="position-absolute w-100 bg-white border rounded shadow" style={{ zIndex: 1050, top: "calc(100% + 2px)", left: 0, minWidth: 320 }}>
                                      <div className="p-2 border-bottom">
                                        <div className="input-group">
                                          <span className="input-group-text bg-white border-end-0"><i className="ti ti-search text-muted" /></span>
                                          <input autoFocus type="text" className="form-control border-start-0 ps-0" placeholder="Search"
                                            value={inputFormatSearch} onChange={(e) => setInputFormatSearch(e.target.value)} />
                                        </div>
                                      </div>
                                      <div style={{ maxHeight: 260, overflowY: "auto" }}>
                                        {inputFormatOptions.filter((o) => o.label.toLowerCase().includes(inputFormatSearch.toLowerCase())).map((o) => (
                                          <div key={o.value}
                                            className={`input-format-option px-3 py-2${inputFormat === o.value ? " is-selected" : ""}`}
                                            onClick={() => { setInputFormat(o.value); setInputFormatOpen(false); setInputFormatSearch(""); setIsDirty(true); }}>
                        ...
                        <>
                          <div className="mb-4">
                            ...
                            <div className="border rounded p-3">
                                ...
                                {dropdownSelected.length > 0 ? (
                                  <>
                                    ...
                                    <button type="button" className="p-0 border-0 bg-transparent text-danger"
                                      onClick={() => { setDropdownOptions((p) => p.filter((o) => !dropdownSelected.includes(o.id))); setDropdownSelected([]); setIsDirty(true); }}>
                                  ...
                                  <>
                                    ...
                                    <div className="d-flex align-items-center gap-3">
                                      <div className="form-check mb-0">
                                        <input className="form-check-input" type="checkbox" id="dd_add_color"
                                          checked={dropdownAddColor} onChange={(e) => { setDropdownAddColor(e.target.checked); setIsDirty(true); }} />
                                        <label className="form-check-label fs-13" htmlFor="dd_add_color">Add color to options</label>
                                      </div>
                                      <button type="button" className="btn btn-sm btn-primary d-flex align-items-center gap-1" onClick={addDropdownOption}>
                                        <i className="ti ti-circle-plus" />Add Options<i className="ti ti-chevron-down" />
                                      </button>
                                    </div>
                                  </>
                                )}
                                ...
                                <SortableContext items={dropdownOptions.map((o) => o.id)} strategy={verticalListSortingStrategy}>
                                  {dropdownOptions.map((opt) => (
                                    <SortableShell key={opt.id} id={opt.id} opacity={opt.active ? 1 : 0.45}>
                                      ...
                                      {dropdownAddColor && (
                                        <div style={{ position: "relative", flexShrink: 0 }}
                                          ...
                                          <button type="button" className="btn p-0 d-flex align-items-center gap-1 border-0" style={{ background: "none" }}
                                            onClick={() => {
                                              if (dropdownColorPickerOpen === opt.id) { setDropdownColorPickerOpen(null); }
                                              else { setDropdownColorPickerOpen(opt.id); setDropdownCustomColorMode(false); setDropdownCustomColorText(opt.color); }
                                            }}>
                                              ...
                                              {!dropdownCustomColorMode ? (
                                                <>
                                                  <div className="d-flex justify-content-end mb-1">
                                                    <button type="button" className="btn-close" style={{ fontSize: 10 }} onClick={() => setDropdownColorPickerOpen(null)} />
                                                  </div>
                                                ...
                                                <>
                                                  <div className="d-flex align-items-center gap-2 mb-2">
                                                    <input type="text" className="form-control form-control-sm" style={{ flex: 1 }}
                                                      value={dropdownCustomColorText} onChange={(e) => setDropdownCustomColorText(e.target.value)} />
                                                    <input type="color" className="border rounded"
                                                  ...
                                                  <div className="d-flex gap-2">
                                                    ...
                                                    <button type="button" className="btn btn-cancel btn-sm" onClick={() => setDropdownCustomColorMode(false)}>Cancel</button>
                                                  </div>
                                                </>
                                              )}
                                            </div>
                                      ...
                                      <input type="text" className="form-control flex-grow-1" value={opt.label}
                                        onChange={(e) => updateDropdownOptionLabel(opt.id, e.target.value)} />

                                      <div className="d-flex align-items-center gap-1 flex-shrink-0">
                                        <button type="button" className="p-0 border-0 bg-transparent text-muted" title="Add Option"
                                        ...
                                        <div style={{ position: "relative" }}
                                          ...
                                          {dropdownMoreMenuOpen === opt.id && (
                                            <div className="bg-white border rounded shadow py-1"
                                              style={{ position: "absolute", zIndex: 1050, right: 0, top: "100%", minWidth: 160 }}>
                                              <button type="button" className="btn btn-sm w-100 text-start px-3 py-2"
                                                onClick={() => { toggleDropdownOptionActive(opt.id); setDropdownMoreMenuOpen(null); }}>
                                                {opt.active ? "Mark as Inactive" : "Mark as Active"}
                                              </button>
                                              <button type="button" className="btn btn-sm w-100 text-start px-3 py-2 text-danger"
                              ...
                              {/* F8 — dropdown option label error */}
                              {errors.dropdown_options && <div className="text-danger small mt-2">{errors.dropdown_options}</div>}
                            </div>
                          </div>
                              ...
                              <div className="col-sm-8">
                                <div className="d-flex align-items-center gap-4">
                                  <div className="form-check mb-0 d-flex align-items-center gap-2">
                                    <input className="form-check-input" type="radio" name="dd_color_placement" id="dd_cp_next"
                                      checked={dropdownColorPlacement === "next"} onChange={() => { setDropdownColorPlacement("next"); setIsDirty(true); }} />
                                  ...
                                  <div className="form-check mb-0 d-flex align-items-center gap-2">
                                    <input className="form-check-input" type="radio" name="dd_color_placement" id="dd_cp_wrap"
                                      checked={dropdownColorPlacement === "wrap"} onChange={() => { setDropdownColorPlacement("wrap"); setIsDirty(true); }} />
                      ...
                      {dataType === "multiselect" && (
                        <div className="mb-4">
                          ...
                          <div className="border rounded p-3">
                            <div className="d-flex align-items-center justify-content-between mb-3">
                              {multiselectSelected.length > 0 ? (
                                <>
                                  ...
                                  <button type="button" className="p-0 border-0 bg-transparent text-danger"
                                    onClick={() => { setMultiselectOptions((p) => p.filter((o) => !multiselectSelected.includes(o.id))); setMultiselectSelected([]); setIsDirty(true); }}>
                                    <i className="ti ti-trash fs-16" />
                                  </button>
                                </>
                              ) : (
                                <>
                                  ...
                                  <button type="button" className="btn btn-sm btn-primary d-flex align-items-center gap-1" onClick={addMultiselectOption}>
                                    <i className="ti ti-circle-plus" />Add Options<i className="ti ti-chevron-down" />
                                  </button>
                                </>
                              )}
                                ...
                                {multiselectOptions.map((opt) => (
                                  <SortableShell key={opt.id} id={opt.id} opacity={opt.active ? 1 : 0.45}>
                                    <input type="checkbox" className="form-check-input flex-shrink-0" style={{ marginTop: 0 }}
                                      ...
                                      onChange={(e) => setMultiselectSelected((p) => e.target.checked ? [...p, opt.id] : p.filter((id) => id !== opt.id))} />
                                    <input type="text" className="form-control flex-grow-1" value={opt.label}
                            ...
                            {/* F8 — multiselect option label error */}
                            {errors.multiselect_options && <div className="text-danger small mt-2">{errors.multiselect_options}</div>}
                          </div>
                        </div>
                      )}
                      ...
                      {dataType === "auto_generate" && (
                        <>
                          <div className="row mb-3 align-items-center">
                            ...
                            <div className="col-sm-8">
                              <input type="text" className="form-control" value={autoPrefix}
                                onChange={(e) => { setAutoPrefix(e.target.value); setIsDirty(true); }} />
                            </div>
                          </div>
                          ...
                          <div className="row mb-3 align-items-center">
                            ...
                            <div className="col-sm-8">
                              <input
                                ...
                                onChange={(e) => { setAutoStartingNumber(e.target.value); setIsDirty(true); setErrors((p) => ({ ...p, starting_number: "" })); }}
                              />
                          ...
                          <div className="row mb-3 align-items-center">
                            ...
                            <div className="col-sm-8">
                              <input type="text" className="form-control" value={autoSuffix}
                                onChange={(e) => { setAutoSuffix(e.target.value); setIsDirty(true); }} />
                            </div>
                          </div>
                          ...
                          <div className="row mb-3 align-items-start">
                            ...
                            <div className="col-sm-8">
                              <div className="p-3 rounded" style={{ backgroundColor: "#fffde7", border: "1px solid #fff9c4" }}>
                                <div className="form-check mb-2">
                                  <input className="form-check-input" type="checkbox" id="auto_add_existing"
                                    checked={autoAddToExisting} onChange={(e) => { setAutoAddToExisting(e.target.checked); setIsDirty(true); }} />
                                ...
                                <div className="d-flex align-items-start gap-2">
                                  ...
                                  <small className="text-muted">This is a one-time setup and you cannot edit this setting later.</small>
                                </div>
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="row mb-3 align-items-center">
                          ...
                          <div className="col-sm-8">
                            {(dataType === "attachment" || dataType === "image") ? (
                              ...
                              <div className="input-group">
                                <span className="input-group-text bg-white"><i className="ti ti-mail text-muted" /></span>
                                <input type="email" className="form-control border-start-0" value={defaultValue}
                                  onChange={(e) => { setDefaultValue(e.target.value); setIsDirty(true); }} placeholder="Enter default email" />
                              </div>
                            ) : dataType === "url" ? (
                              <div className="input-group">
                                <span className="input-group-text bg-white"><i className="ti ti-world text-muted" /></span>
                                <input type="url" className="form-control border-start-0" value={defaultValue}
                                  onChange={(e) => { setDefaultValue(e.target.value); setIsDirty(true); }} placeholder="Enter default URL" />
                              </div>
                            ) : dataType === "phone" ? (
                              <div className="input-group">
                                <span className="input-group-text bg-white"><i className="ti ti-phone text-muted" /></span>
                                <input type="tel" className="form-control border-start-0" value={defaultValue}
                                  onChange={(e) => { setDefaultValue(e.target.value); setIsDirty(true); }} placeholder="Enter default phone" />
                              </div>
                            ) : dataType === "amount" ? (
                              <div className="input-group">
                                <span className="input-group-text bg-white">INR</span>
                                <input type="number" className="form-control border-start-0" value={defaultValue}
                                  onChange={(e) => { setDefaultValue(e.target.value); setIsDirty(true); }} placeholder="0.00" />
                              </div>
                            ) : dataType === "percent" ? (
                              <div className="input-group">
                                <input type="number" className="form-control border-end-0" value={defaultValue}
                                  onChange={(e) => { setDefaultValue(e.target.value); setIsDirty(true); }} />
                                <span className="input-group-text bg-white">%</span>
                              </div>
                            ) : dataType === "dropdown" ? (
                              <select className="form-select" value={dropdownDefault} onChange={(e) => { setDropdownDefault(e.target.value); setIsDirty(true); }}>
                                <option value=""></option>
                              ...
                              <select className="form-select" value={multiselectDefault} onChange={(e) => { setMultiselectDefault(e.target.value); setIsDirty(true); }}>
                                <option value=""></option>
                              ...
                              <div className="form-check mb-0">
                                <input className="form-check-input" type="checkbox" id="checkbox_default"
                                  checked={checkboxDefault} onChange={(e) => { setCheckboxDefault(e.target.checked); setIsDirty(true); }} />
                                <label className="form-check-label" htmlFor="checkbox_default">Ticked by default</label>
                              </div>
                            ) : dataType === "datetime" ? (
                              <CommonDatePicker value={datetimeDefaultValue} onChange={(date) => { setDatetimeDefaultValue(date ?? null); setIsDirty(true); }}
                                showTime={{ format: "HH:mm", minuteStep: 30 }} format="DD/MM/YYYY HH:mm" />
                            ) : dataType === "date" ? (
                              <div>
                                {dateCustomMode ? (
                                  <div>
                                    <CommonDatePicker value={dateCustomValue} onChange={(date) => { setDateCustomValue(date ?? null); setIsDirty(true); }} format="DD/MM/YYYY" />
                                    <Link to="#" className="text-primary fs-13 d-block mt-1"
                                  ...
                                  <div ref={dateDefaultRef} style={{ position: "relative" }}>
                                    ...
                                    {dateDefaultOpen && (
                                      <div className="position-absolute bg-white border rounded shadow" style={{ zIndex: 1050, top: "calc(100% + 2px)", left: 0, minWidth: 280 }}>
                                        <div className="p-2 border-bottom">
                                          <div className="input-group">
                                            <span className="input-group-text bg-white border-end-0"><i className="ti ti-search text-muted" /></span>
                                            <input autoFocus type="text" className="form-control border-start-0 ps-0" placeholder="Search"
                                              value={dateDefaultSearch} onChange={(e) => setDateDefaultSearch(e.target.value)} />
                                          </div>
                                        </div>
                                        <div style={{ maxHeight: 220, overflowY: "auto" }}>
                                          {relativeDateOptions.filter((o) => o.label.toLowerCase().includes(dateDefaultSearch.toLowerCase())).map((o) => (
                                            <div key={o.value}
                                              className={`input-format-option px-3 py-2${dateDefaultValue === o.value ? " is-selected" : ""}`}
                                              onClick={() => { setDateDefaultValue(o.value); setDateDefaultOpen(false); setDateDefaultSearch(""); setIsDirty(true); }}>
                                              {o.label}
                                            </div>
                                          ))}
                                        </div>
                              ...
                              <textarea className="form-control" rows={3} value={defaultValue}
                                onChange={(e) => { setDefaultValue(e.target.value); setIsDirty(true); }} />
                            ) : (
                      ...
                      {/* Show when creating transactions */}
                      {supportsShowInTx && (
                        <div className="row mb-3 align-items-center">
                            ...
                            <div className="d-flex align-items-center gap-3">
                              <div className="form-check mb-0">
                                <input className="form-check-input" type="radio" name="add_show_tx" id="add_show_tx_yes"
                                  checked={showInTx === true} onChange={() => { setShowInTx(true); setIsDirty(true); }} />
                              ...
                              <div className="form-check mb-0">
                                <input className="form-check-input" type="radio" name="add_show_tx" id="add_show_tx_no"
                                  checked={showInTx === false} onChange={() => { setShowInTx(false); setIsDirty(true); }} />
                        ...
                        <div className="row mb-3 align-items-start">
                          ...
                          <div className="col-sm-8">
                            <div className="position-relative" ref={modulesRef}>
                              ...
                              {modulesOpen && (
                                <div className="position-absolute w-100 bg-white border rounded shadow" style={{ zIndex: 1050, bottom: "calc(100% + 2px)", left: 0 }}>
                                  <div className="p-2 border-bottom">
                                    <div className="input-group">
                                      <span className="input-group-text bg-white border-end-0"><i className="ti ti-search text-muted" /></span>
                                      <input autoFocus type="text" className="form-control border-start-0 ps-0" placeholder="Search"
                                        value={modulesSearch} onChange={(e) => setModulesSearch(e.target.value)} />
                                    </div>
                                  </div>
                                  <div className="px-3 py-2 border-bottom">
                                    <div className="form-check mb-0">
                                      <input className="form-check-input" type="checkbox" id="mod_all"
                                        checked={includeModules.length === includeModuleOptions.length}
                                        onChange={(e) => { setIncludeModules(e.target.checked ? includeModuleOptions.map((o) => o.value) : []); setIsDirty(true); }} />
                                      <label className="form-check-label fw-medium" htmlFor="mod_all">All</label>
                                    </div>
                                  </div>
                                  <div style={{ maxHeight: 260, overflowY: "auto" }}>
                                    {(["SALES", "PURCHASES"] as const).map((group) => {
                                      const filtered = includeModuleOptions.filter((o) => o.group === group && o.label.toLowerCase().includes(modulesSearch.toLowerCase()));
                                        ...
                                        <div key={group}>
                                          ...
                                          {filtered.map((o) => (
                                            <div key={o.value} className="px-3 py-2">
                                              <div className="form-check mb-0">
                                                <input className="form-check-input" type="checkbox" id={`mod_${o.value}`}
                                                  checked={includeModules.includes(o.value)}
                                                  onChange={(e) => {
                                                    setIncludeModules((p) => e.target.checked ? [...p, o.value] : p.filter((v) => v !== o.value));
                                                    setIsDirty(true);
                                                  }} />
                      ...
                      {/* Show in All PDFs */}
                      {!["auto_generate","text_multi","dropdown","multiselect","lookup","attachment","image"].includes(dataType) && (
                        <div className="row mb-3 align-items-center">
                            ...
                            <div className="d-flex align-items-center gap-3">
                              <div className="form-check mb-0">
                                <input className="form-check-input" type="radio" name="add_show_pdfs" id="add_show_pdfs_yes"
                                  checked={showInPdfs === true} onChange={() => { setShowInPdfs(true); setIsDirty(true); }} />
                              ...
                              <div className="form-check mb-0">
                                <input className="form-check-input" type="radio" name="add_show_pdfs" id="add_show_pdfs_no"
                                  checked={showInPdfs === false} onChange={() => { setShowInPdfs(false); setIsDirty(true); }} />
                          ...
                          <div className="d-flex align-items-center gap-3">
                            <div className="form-check mb-0">
                              <input className="form-check-input" type="radio" name="add_mandatory" id="add_mandatory_yes"
                                checked={mandatory === true} onChange={() => { setMandatory(true); setIsDirty(true); }} />
                            ...
                            <div className="form-check mb-0">
                              <input className="form-check-input" type="radio" name="add_mandatory" id="add_mandatory_no"
                                checked={mandatory === false} onChange={() => { setMandatory(false); setIsDirty(true); }} />
                      ...
                      {/* Preview */}
                      {!["number","decimal","amount","percent","date","datetime","checkbox","auto_generate","dropdown","multiselect","lookup","attachment","image"].includes(dataType) && (
                        <div className="row mb-3 align-items-start">
                            ...
                            <div className="border rounded p-3">
                              <div className="row align-items-center">
                                ...
                                <div className="col-sm-7">
                                  {dataType === "email" ? (
                                    <div className="input-group">
                                      <span className="input-group-text bg-white border-end-0"><i className="ti ti-mail text-muted" /></span>
                                      <input type="email" className="form-control border-start-0" disabled />
                                    </div>
                                  ) : dataType === "url" ? (
                                    <div className="input-group">
                                      <span className="input-group-text bg-white border-end-0"><i className="ti ti-world text-muted" /></span>
                                      <input type="url" className="form-control border-start-0" disabled />
                                    </div>
                                  ) : dataType === "phone" ? (
                                    <div className="input-group">
                                      <span className="input-group-text bg-white border-end-0"><i className="ti ti-phone text-muted" /></span>
                  ...
                  {/* Is Mandatory — shown before a data type is selected */}
                  {!dataType && (
                    <div className="row mb-3 align-items-center">
                      <label className="col-sm-4 col-form-label fw-medium fs-14">Is Mandatory</label>
                      <div className="col-sm-8">
        ...
        {/* F5 — Save button is btn-primary (not btn-danger) */}
        <div className="bg-white border-top d-flex align-items-center gap-2 px-4" style={{ position: "sticky", bottom: 0, zIndex: 100, height: 60 }}>
          ...
          <button type="button" className="btn btn-outline-light" onClick={handleCancel} disabled={saving}>
            Cancel
          </button>
        </div>
